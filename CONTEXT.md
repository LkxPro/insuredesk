...
### SLAPolicy（SLA 策略）
SLA 规则的结构化配置，按投诉等级各一条，管理员可编辑（全量可配置提醒引擎，见 ADR 0005）。含首响违约线、超时时长、以及一个可增删的类型化提醒规则列表。

**首响违约线（firstResponseMinutes）**：单一数值（一般120/高级120/加急60/特急30 分钟），仅作"待首响告警"的染红阈值——不再是一条提醒规则。

**两种提醒规则类型**（本期作为"我的待办"读时计算的判定谓词）：
- `follow_up_checkpoint`：跟进检查点（到某时间点前累计跟进不足则进入待办，含提前量）
- `rolling_follow_up`：滚动提醒（特急专用，距上次跟进每 N 小时进入待办）

完整字段与各级默认值见 PRD §3.8。本期无后台定时任务，全部由前端 30 秒轮询时读时计算（见 ADR 0004）。
...
**不落库、读时派生**：无 `follower` 字段。当前跟进人 = `assigneeId` JOIN `users.name`（永远取最新名，避免改名后陈旧）；"谁跟进过"（历史交接链）看 ProcessLog 时间线（assign 的 from/to 姓名快照 + 每条 comment 的 operatorName）。
...
面向单个用户的提醒。**两轨制**（详见 ADR 0004）：

- **轨 1 — 收件箱（存储）**：只承载**用户操作触发的真事件**，本期仅 `assigned`（被分配/改派工单时通知新责任人，覆盖首次分配与改派）——操作发生时同步写入 AppNotification，有已读/未读与 toast。
- **轨 2 — 我的待办（不落库，读时计算）**：`overdue`（已超时）/ `due_soon`（快超时）/ 待首响 / 检查点未达 / 特急欠跟进等**时间类告警**不写库，由前端 30 秒轮询时按当前 assigneeId 名下工单的 `dueAt`/`now`/`lastCommentAt`/comment 累计数**实时计算**，以红点计数呈现。
...
**为何时间类告警不落库**：它们是随 `now` 持续变化的派生条件（ADR 0001 已确立"显示状态读时算"），非离散事件。读时计算天然带来：改派后接收人随当前 assigneeId 切换（消除"接收人冻结"）、改等级后错过的检查点不补发、无需去重表。本期无后台定时任务；二期加带外推送（邮件/飞书）时再引入定时任务专供带外检测。

**送达方式**：前端每 30 秒轮询——拉取 `assigned` 未读通知（轨 1）+ 计算当前用户的待办告警（轨 2），驱动红点 / toast。

## 业务规则

### 通知触发规则
- **assigned（操作触发，轨 1）**：分配 / 改派工单时同步写入 AppNotification，targetUserId = 新责任人
- **时间类告警（读时计算，轨 2）**：前端轮询时按当前 assigneeId 名下工单实时算，无生成、无去重
  - **未分配工单（assigneeId=null）不进任何人的待办**——待办查询带 assigneeId=me，未分配单天然不落入；超时/预警靠看板呈现，主管主动处理
  - **无需去重标记**：告警是"当前条件是否为真"，条件仍真就一直显示，条件消解（跟进/完结）即掉出，天然幂等
  - 待办判定与数据看板"已超时数 / 2小时预警数"复用同一时间判断条件（收敛到单一函数/视图）
