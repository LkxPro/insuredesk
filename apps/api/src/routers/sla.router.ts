...
/**
 * SLA 策略配置 routes (issue #33): thin wrappers per ADR 0006 — the shared
 * input schema is the whole write contract (wire shape from Zod, every
 * business rule from the rule-engine's canonical validateSlaPolicy, issue
 * #47), business logic in sla.service. sla.view / sla.edit are held only by
 * the 管理员 preset (PRD §3.8 "管理员可编辑").
 */
