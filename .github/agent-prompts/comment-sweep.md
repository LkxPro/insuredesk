...
Repository rule (AGENTS.md): a comment is allowed only when it states a constraint the code itself cannot express — why an alternative implementation is impossible, an external system's implicit contract, or the reason a workaround exists. Delete every comment that:
...
- explains a "why" that the code already makes obvious.
