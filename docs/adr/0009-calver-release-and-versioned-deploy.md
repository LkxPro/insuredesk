...
- 首次发版前将现有全部迁移 squash 成干净基线(最后一次行使"可改写历史
  迁移"的活跃开发期约定)。
- **自首个 release tag 起该约定废止**:迁移 append-only,不再改写历史;
  schema 变更须携带存量数据迁移;不再假设"重建库即可"。
...
main 设分支保护(禁直推、CI 绿才可合入);CI 增加生产 Dockerfile 构建校验
(只构建不推送),避免镜像坏到发版才发现;启用 Dependabot 自动依赖升级 PR。
