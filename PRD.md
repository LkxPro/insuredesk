...
- **角色管理**：按权限点清单配置角色（页面、数据、操作权限），管理员为唯一系统角色
- **权限体系**：
  - 页面权限：访问特定页面
  - 数据权限：查看全部/团队/个人工单
  - 操作权限：新增、编辑、分配、导出等
...
| attachments | Attachment[] | 附件列表(本期不做)   |
...
| system      | boolean  | ✅   | 系统角色标记（仅管理员为 true） |


**角色模型**：**管理员**是唯一的系统角色，名称、权限、删除全锁；其余一切角色均可改名、改权限、删除（删除受"角色使用中"约束）。系统首次初始化时出厂生成下列角色，此后它们与手建角色无任何区别，改动和删除永不被系统干预：
...
1. **手动分配**：
  - 主管/管理员选择工单 → 选择责任人（任意**启用**用户，不要求当日在班）→ 确认分配
  - 系统自动：
    - 更新 assigneeId
    - 记录 assignedAt（首次分配时）
...
**手动分配与排班相互独立（正式规则，#42）**：手动分配、批量分配与改派**不受排班限制**——拥有分配权限的主管/管理员可将工单分给任意**启用**用户，与该用户当日是否配置排班、班次时段、工单渠道均无关。排班（§3.6）**只**决定"按排班自动分配"的候选人范围，不是手动分配的前置校验。唯一的人选硬约束是用户必须处于启用状态：已停用用户不能接收任何新分配或改派（存量在手工单不受影响）。后续迭代不得把自动分配的在岗校验加到手动分配路径上。
...
核心取向一句话：**TypeScript 同构**——pnpm monorepo（`apps/api` Fastify + tRPC + Prisma + PostgreSQL；`apps/web` React + Vite + shadcn + TanStack Query；`packages/shared` 共享领域类型 + Zod schema + 枚举），常驻后端进程（非 Serverless），Session + httpOnly Cookie 认证（SSO-ready）。

ADR 未覆盖的补充细节：

- 路由：React Router
- 附件对象存储：方案随部署环境确定（如 S3 兼容对象存储）（本期不做）
...
| v2.4 | 2026-07-10 | 固化手动分配不受排班限制（#42）：§4.3 明确手动分配/批量分配/改派可指向任意启用用户，与排班日期/班次/渠道无关；排班只决定"按排班自动分配"的候选人范围；已停用用户不能接收新分配或改派。同步 CONTEXT.md 词汇表，补 API 与前端回归测试 | -   |
