...
- **通知推送**：站内通知（assigned/overdue/due_soon），30 秒轮询送达（见 ADR 0004）
...
| --------------- | -------- | --- | ---------------------------------------------------------------------- | ------ |
| id              | string   | ✅   | -                                                                      | 通知ID   |
| type            | enum     | ✅   | assigned / overdue / due_soon                                | 通知类型（3种）   |
| title           | string   | ✅   | -                                                                      | 通知标题   |
| content         | string   | ✅   | -                                                                      | 通知内容   |
...
- 判定时刻**唯一**：在 `createdAt + checkpointHours − advanceMinutes` 这一刻判定一次；累计 comment 数 < requiredCount 则发提醒。**每条 checkpoint 规则一生只在这一刻判定、最多发一次**（与"幂等：每条提醒只发一次"一致）
...
> **注意区分两个"首响相关"的值**（二者语义不同，不是笔误）：
> - `firstResponseMinutes`（SLAPolicy 主体）= **首响时限/违约线**（一般120/高级120/加急60/特急30 分钟），用于判定"是否首响超时"
> - `first_response` 提醒的 `afterMinutes`（一般90/高级90/加急30/特急15 分钟）= **提前催办的提醒线**，比违约线更早触发，提醒责任人"再不首响就要违约了"
> - 因此 afterMinutes < firstResponseMinutes 是正常设计（提前催办）
...
- **接收人**：当前 assigneeId；**工单未分配时（assigneeId=null）不发送提醒通知**——超时/预警靠数据看板呈现，由主管主动处理
...
**首响的用途边界**：首响时限仅用于**驱动首响提醒**（催办），**本期不纳入数据看板与客服考核统计**（考核仅统计处理单量/处理时效/超时情况三维，见 §2.2）。
