...
start_head=$(git -C "$worktree" rev-parse HEAD)
...
if [ -z "$failure" ]; then
  ...
  if [ "$mode" = brief ] && ! "$script_dir/agent/materialize-plan.sh" "$issue" "$worktree/$spec_dir/tickets.json"; then
